...
class Bot {
    ...
    public async commands(){
        this.client.on('message', async (message: Message) => {
            ...
            switch (msg) {
                case "1":
                    await this.sendProjectInfo(message);
                    break;
                case "2":
                    await this.sendTeamInfo(message);
                    break;
                default:
                    await this.sendUnkownCommand(message);
                    break; 
            }
    ...
    private async sendProjectInfo(message: Message){
